...
const studentSchema = z.object({
  ...
  level_kelas: z.string().min(1, "Tingkat kelas harus dipilih"),
...
const StudentForm = ({
  ...
  const [allKelasForKejuruan, setAllKelasForKejuruan] = useState<Kelas[]>([]);
  ...
  useEffect(() => {
    if (initialData && kejuruans.length > 0) {
      const kejuruanValue = initialData.kejuruan;
      const matchingKejuruan = kejuruans.find(
        (k) => k.id === kejuruanValue || k.label_jurusan === kejuruanValue
      );

      form.reset({
        nis: initialData.nis || "",
        nama: initialData.fullName || "",
        kejuruan: matchingKejuruan ? matchingKejuruan.id : "",
        kelas: initialData.kelas || "",
        tempatLahir: initialData.birthPlace || "",
        tanggalLahir: initialData.birthDate
      ...
      if (matchingKejuruan) {
        setSelectedKejuruan(matchingKejuruan.id);
      }

      if (initialData.kelas && matchingKejuruan) {
        const setInitialLevel = async () => {
          const { data: classData } = await supabase
            .from("kelas")
            .select("level_kelas")
            .eq("id", initialData.kelas)
            .single();
          if (classData) {
            form.setValue("level_kelas", classData.level_kelas);
          }
        };
        setInitialLevel();
      }
  ...
  }, [initialData, kejuruans, form]);

  useEffect(() => {
    const fetchKejuruans = async () => {
      const { data, error } = await supabase
        .from("kejuruan")
        .select("id, nama_jurusan, label_jurusan");
      if (error) {
        toast.error("Gagal memuat daftar kejuruan.");
        console.error(error);
      } else {
        setKejuruans(data as Kejuruan[]);
      }
    };
    fetchKejuruans();
  }, []);
    ...
    const fetchKelas = async () => {
      if (selectedKejuruan) {
        const { data, error } = await supabase
          .from("kelas")
          .select("*")
          .eq("id_kejuruan", selectedKejuruan);
        if (error) {
          toast.error("Gagal memuat daftar kelas.");
          console.error(error);
        } else {
          setAllKelasForKejuruan(data as Kelas[]);
        }
      } else {
        setAllKelasForKejuruan([]);
      }
    };
    fetchKelas();
  }, [selectedKejuruan]);
  ...
  const onSubmit = async (data: StudentFormData) => {
    ...
    const selectedKejuruanObject = kejuruans.find(
      (k) => k.id === data.kejuruan
    );
    const selectedKelasObject = allKelasForKejuruan.find(
      (k) => k.id === data.kelas
    );
    ...
    const studentDataForSupabase = {
      ...
      kejuruan: selectedKejuruanObject?.nama_jurusan,
      level_kelas: data.level_kelas,
      kelas: selectedKelasObject?.nama_kelas,
      id_kelas: data.kelas,
  ...
  const selectedLevel = form.watch("level_kelas");
  const uniqueLevels = [
    ...new Set(allKelasForKejuruan.map((k) => k.level_kelas)),
  ];
  const filteredKelasNames = allKelasForKejuruan.filter(
    (k) => k.level_kelas === selectedLevel
  );
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) => {
                  form.setValue("kejuruan", value);
                  setSelectedKejuruan(value);
                  form.setValue("level_kelas", "");
                  form.setValue("kelas", "");
                }}
              ...
              {form.formState.errors.kejuruan && (
                <p className="text-sm text-destructive">
                  {form.formState.errors.kejuruan.message}
                </p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="level_kelas" className="text-sm font-medium">
                Tingkat Kelas *
              </Label>
              <Select
                onValueChange={(value) => {
                  form.setValue("level_kelas", value);
                  form.setValue("kelas", "");
                }}
                value={form.watch("level_kelas") || ""}
                disabled={!selectedKejuruan}
              >
                <SelectTrigger className="transition-smooth focus:ring-2 focus:ring-primary/20">
              ...
              {form.formState.errors.level_kelas && (
                <p className="text-sm text-destructive">
                  {form.formState.errors.level_kelas.message}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) => form.setValue("kelas", value)}
                value={form.watch("kelas")}
                disabled={!form.watch("level_kelas")}
              >
                <SelectTrigger className="transition-smooth focus:ring-2 focus:ring-primary/20">
                  <SelectValue placeholder="Pilih nama kelas" />
                </SelectTrigger>
                <SelectContent>
                  {filteredKelasNames.map((k) => (
                    <SelectItem key={k.id} value={k.id}>
                      {k.nama_kelas}
                    </SelectItem>
                  ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !form.watch("tanggalLahir") && "text-muted-foreground"
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {form.watch("tanggalLahir") ? (
                      format(form.watch("tanggalLahir"), "dd MMMM yyyy")
                    ) : (
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={form.watch("tanggalLahir")}
                    onSelect={(date) =>
                      form.setValue("tanggalLahir", date as Date)
                    }
                    initialFocus
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) => form.setValue("jenisKelamin", value)}
              >
                <SelectTrigger className="transition-smooth focus:ring-2 focus:ring-primary/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              {form.formState.errors.agama && (
                <p className="text-sm text-destructive">
                  {form.formState.errors.agama.message}
                </p>
              )}
            </div>
